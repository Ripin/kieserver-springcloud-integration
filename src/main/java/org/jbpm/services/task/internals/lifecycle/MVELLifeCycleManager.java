...
public class MVELLifeCycleManager implements LifeCycleManager {
	...
	void evalCommand(final Operation operation, final List<OperationCommand> commands, final Task task, final User
            user, final OrganizationalEntity targetEntity, List<String> groupIds, OrganizationalEntity... entities)
            throws PermissionDeniedException {
		String errorMessage = "";
		boolean statusMatched = false;
		errorMessage = "commandsSize=" + commands.size();
		...
		for (OperationCommand command : commands) {
				...
				for (Status status : command.getStatus()) {
					if (task.getTaskData().getStatus() == status) {
						...
						if (!isAllowed(command, task, user, groupIds)) {
							errorMessage += ",User '" + user + "' does not have permissions to execute operation '" +
						...
						errorMessage += ",No match on status for task " + task.getId() + ":status {" + task.getTaskData
                                ().getStatus() + "}  != {" + status + "}";
						logger.debug("No match on status for task {} :status {}  != {}", task.getId(), task
                                .getTaskData().getStatus(), status);
					}
				}
			}
			...
			if (command.getPreviousStatus() != null) {
				for (Status status : command.getPreviousStatus()) {
					if (taskData.getPreviousStatus() == status) {
						...
						errorMessage += "No match on previous status for task {" + task.getId() + "}:status {" + task
								.getTaskData
								().getStatus() + "}  != {" + status + "}";
						logger.debug("No match on previous status for task {} :status {}  != {}", task.getId(), task
                                .getTaskData().getStatus(), status);
					}
				}
			}
			...
			if (!command.isGroupTargetEntityAllowed() && targetEntity instanceof Group) {
				errorMessage += "User '" + user + "' was unable to execute operation '" + operation + "' on task" +
                        " id " + task.getId() + " due to 'target entity cannot be group'";
				throw new PermissionDeniedException(errorMessage);
			}
		}
		if (!statusMatched) {
			errorMessage += "User '" + user + "' was unable to execute operation '" + operation + "' on task id " +
                    "" + task.getId() + " due to a no 'current status' match";
			throw new PermissionDeniedException(errorMessage);
		}
	...
	private boolean isAllowed(final OperationCommand command, final Task task, final User user, List<String>
            groupIds) {

		boolean operationAllowed = false;
		boolean isExcludedOwner = ((InternalPeopleAssignments) task.getPeopleAssignments()).getExcludedOwners()
                .contains(user);
		...
		for (Allowed allowed : command.getAllowed()) {
			...
			switch (allowed) {
				case Owner: {
					operationAllowed = !isExcludedOwner && (task.getTaskData().getActualOwner() != null && task
                            .getTaskData().getActualOwner().equals(user));
					break;
				}
				case Initiator: {
					operationAllowed = (!isExcludedOwner && task.getTaskData().getCreatedBy() != null && (task
                            .getTaskData().getCreatedBy().equals(user) || groupIds != null && groupIds.contains(task
                            .getTaskData().getCreatedBy().getId())));
					break;
				}
				case PotentialOwner: {
					operationAllowed = !isExcludedOwner && isAllowed(user, groupIds, (List<OrganizationalEntity>) task
                            .getPeopleAssignments().getPotentialOwners());
					break;
				}
				case BusinessAdministrator: {
					operationAllowed = isAllowed(user, groupIds, (List<OrganizationalEntity>) task
                            .getPeopleAssignments().getBusinessAdministrators());
					break;
				}
				case TaskStakeholders: {
					operationAllowed = !isExcludedOwner && isAllowed(user, groupIds, (List<OrganizationalEntity>) (
	...
	private boolean isAllowed(final User user, final List<String> groupIds, final List<OrganizationalEntity>
            entities) {
	...
	private void commands(final OperationCommand command, final Task task, final User user, final OrganizationalEntity
            targetEntity, OrganizationalEntity... entities) {
			...
			switch (command.getExec()) {
				...
				case Nominate: {
					if (entities != null && entities.length > 0) {
						List<OrganizationalEntity> potentialOwners = new ArrayList<OrganizationalEntity>(Arrays.asList
                                (entities));
	...
	public void taskOperation(final Operation operation, final long taskId, final String userId, final String
            targetEntityId, final Map<String, Object> data, List<String> groupIds, OrganizationalEntity... entities)
	...
	protected ExecutionErrorHandler getExecutionErrorHandler() {
		ExecutionErrorManager errorManager = (ExecutionErrorManager) ((org.jbpm.services.task.commands.TaskContext)
                context).get(EnvironmentName.EXEC_ERROR_MANAGER);
